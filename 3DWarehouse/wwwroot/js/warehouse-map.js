...
(function () {
  ...
  // Route có {warehouseId} (vd /Warehouse/pmc/LayoutData) — view tiêm window.PMC_WAREHOUSE_ID,
  // mọi fetch() bên dưới ghép chuỗi này vào thay vì hard-code 1 kho.
  ...
  function loadPage(p) {
    ...
    fetch('Warehouse/' + WH_ID + '/LocationMaterials?id=' + curLoc + '&page=' + p)
  ...
  function doSearch(q) {
    ...
    fetch('Warehouse/' + WH_ID + '/Find?barcode=' + encodeURIComponent(q))
  ...
  function refreshLayout() {
    fetch('Warehouse/' + WH_ID + '/LayoutData').then(function (r) { return r.json(); }).then(function (d) { applyOccupancy(d || []); }).catch(function () {});
  }
  if (window.signalR) {
    try {
